-- bulk_import_receivables RPC を daily_sales v2 (vehicle3_sales 廃止 + labor_cost 追加) に対応させる。
-- p_staff_sales は後方互換のためシグネチャに残す (空配列で渡せば INSERT 0 件)。
...
BEGIN
  ...
  -- daily_sales INSERT (vehicle3_sales 廃止 / labor_cost 追加)
  WITH ins AS (
    INSERT INTO public.daily_sales (
      ...
      expense_note, expense_amount, labor_cost, cash,
      source_file
    )
    SELECT
      (r ->> 'work_date')::DATE,
  ...
  -- daily_staff_sales (後方互換: 渡されれば INSERT、空なら 0 件)
  WITH ins AS (
